...
function send_configuration_property {
    suffix=$5
    local command="property e:stressed-configurations"${suffix}" k:id=$1 t:cpu  d:"'"'$(get_current_iso_time)'"'" v:start_time="$2" v:end_time="$3" v:options="'"'$4'"';
    send_network_command ${command}
}
...
for ((i = 0; i <= $core_count; i++)); do
    ...
    if [[ ${i} > 0 ]]; then
        docker run --name ${container_name} ${constraint} -it progrium/stress ${options} --timeout $(calc $1+$2)
    else
...
clean_up
for ((i = 0; i <= $core_count; i++)); do
    echo ${i}" constraint configuration is started";
    options="--cpu "${i}"";
    echo "Warm up!";
    start_time=$(get_current_iso_time);
    if [[ ${i} > 0 ]]; then
    ...
    echo ${end_time}
    echo "Sending statistic about the configuration";
    send_configuration_property conf${i} ${start_time} ${end_time} "$options" "-constraint"
done